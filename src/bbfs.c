...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    char *contents_;
    char cmd[512];
    for (int i=0; i < sizeof(contents)/HASH_SIZE; i++)
    {
         
        for (int j=i; j < HASH_SIZE; j++)
        {
            contents_ = contents_ + contents[i];
        }
    ...
    return PointerToInt(act_contents);
